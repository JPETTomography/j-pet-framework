...
public:
  ...
  /**
   * @brief Get the JPetPMSignal object from which this RecoSignal was created
   * Return a dummy object if raw signal was not set (e.g. for scope signals)
   */
  const JPetPMSignal& getPMSignal() const { return fPMSignal; }

  void setPMSignal(const JPetPMSignal& pmSignal);
