...
{
  size_t res = 0;
  while (layer.getRadius() != fRadii[res]) res++;
  return res + 1;
}
...
size_t LargeBarrelMapping::getSlotNumber(const JPetBarrelSlot& slot) const
{
  ...
  size_t res = 0;
  while (slot.getTheta() != theta[res]) res++;
  return res + 1;
}

size_t LargeBarrelMapping::calcDeltaID(const JPetBarrelSlot& slot1,
...
LargeBarrelTask::LargeBarrelTask(const char* name): JPetUserTask(name) {}
...
TOT_Hists::TOT_Hists(const char* name): LargeBarrelTask(name) {}

TOT_Hists::~TOT_Hists() {}

void TOT_Hists::createTOTHistos(
  const std::string& suffix, const size_t bins,
    ...
    for (size_t sl = 1, n = map()->getSlotsCount(ln); sl <= n; sl++) {
      for (size_t thr = 1; thr <= 4; thr++) {
        getStatistics().createHistogram(
          new TH1F(("TOT-" + LayerSlotThr(ln, sl, thr) + "-A-" + suffix).c_str(),
          "", bins, min, max));
        getStatistics().createHistogram(
          new TH1F(("TOT-" + LayerSlotThr(ln, sl, thr) + "-B-" + suffix).c_str(),
          "", bins, min, max));
      }
    }
  }
...
void TOT_Hists::fillTOTHistos(const JPetHit& hit, const std::string& suffix)
{
  ...
  auto TOTA = hit.getSignalA().getRecoSignal()
    .getRawSignal().getTOTsVsThresholdNumber();
  auto TOTB = hit.getSignalB().getRecoSignal()
    .getRawSignal().getTOTsVsThresholdNumber();
  for (size_t thr = 1; thr <= 4; thr++) {
    getStatistics().getHisto1D(
      ("TOT-" + LayerSlotThr(strip1.layer, strip1.slot, thr) + "-A-" + suffix).c_str()
    )->Fill(TOTA[thr]);
    getStatistics().getHisto1D(
      ("TOT-" + LayerSlotThr(strip1.layer, strip1.slot, thr) + "-B-" + suffix).c_str()
    )->Fill(TOTB[thr]);
  }
}
