...
Synchronization::Synchronization(
  const shared_ptr <JPetGeomMappingInterface> map,
  istream& str,
  const Synchronization::TimeCalculation timecalc
...
{
  const auto& offsets = f_offsets->operator[](
    f_mapping->getStripPos(hit.getBarrelSlot())
  );
  map<int, double> lead_times_A = hit.getSignalA().getRecoSignal()
    .getRawSignal().getTimesVsThresholdNumber(JPetSigCh::Leading);
  map<int, double> lead_times_B = hit.getSignalB().getRecoSignal()
    .getRawSignal().getTimesVsThresholdNumber(JPetSigCh::Leading);
  vector<double> A, B;
...
/**
 * Returns the function needed for the measured signal time estimation.
 * If list contains one number it means that one threshold will be used.
 * Otherwise the average of given thresholds will be taken.
