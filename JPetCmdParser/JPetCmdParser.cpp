...
std::vector<JPetOptions> JPetCmdParser::parseAndGenerateOptions(int argc, const char** argv)
{
  ...
  if(variablesMap.count("json")){
    ...
    for(auto const& iter : optionsFromJson.getOptions()){
      if((((variablesMap.insert(std::make_pair<std::string, std::string>(iter.first, iter.second))).second)==false) && (iter.second != variablesMap[iter.first].as< std::vector<std::string> >())){
