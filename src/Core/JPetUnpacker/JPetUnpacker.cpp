...
void JPetUnpacker::setParams(const std::string& hldFile, int numOfEvents, const std::string& cfgFile, const std::string& totCalibFile,
...
bool JPetUnpacker::exec()
{
  ...
  fUnpacker->UnpackSingleStep(fHldFile.c_str(), fCfgFile.c_str(), fEventsToProcess, refChannelOffset, fTOTCalibFile.c_str(), fTDCCalibFile.c_str());
  return true;
}
