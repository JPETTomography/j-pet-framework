 ...
 */
JPetHit::JPetHit(float energy, float qualityOfEnergy, float time, float qualityOfTime, TVector3& position, JPetPhysSignal& signalA,
                 JPetPhysSignal& signalB, JPetBarrelSlot& barreSlot, JPetScin& scin)
    : TObject(), fFlag(JPetHit::Unknown), fEnergy(energy), fQualityOfEnergy(qualityOfEnergy), fTime(time), fQualityOfTime(qualityOfTime),
      fPos(position), fSignalA(signalA), fSignalB(signalB), fBarrelSlot(&barreSlot), fScintillator(&scin)
{
  fIsSignalAset = true;
  fIsSignalBset = true;
  if (!checkConsistency())
 ...
 */
JPetHit::RecoFlag JPetHit::getRecoFlag() const { return fFlag; }

/**
 * Get hit energy in [keV]
 */
float JPetHit::getEnergy() const { return fEnergy; }

/**
 * Get the quality of the hit energy in [kev]
 */
float JPetHit::getQualityOfEnergy() const { return fQualityOfEnergy; }

/**
 * Get the reconstructed time of the hit in [ps]
 */
float JPetHit::getTime() const { return fTime; }

/**
 * Get the time difference of the two signals, that construct the hit, unit is [ps]
 */
float JPetHit::getTimeDiff() const { return fTimeDiff; }

/**
 * Get the quality of the hit time in [ps]
 */
float JPetHit::getQualityOfTime() const { return fQualityOfTime; }

/**
 * Get the quality of the hit signals time difference in [ps]
 */
float JPetHit::getQualityOfTimeDiff() const { return fQualityOfTimeDiff; }

/**
 * Get x-axis hit position
 */
float JPetHit::getPosX() const { return fPos.X(); }

/**
 * Get y-axis hit position
 */
float JPetHit::getPosY() const { return fPos.Y(); }

/**
 * Get z-axis hit position
 */
float JPetHit::getPosZ() const { return fPos.Z(); }

/**
 * Get position along the axis indicated by index argument
 */
float JPetHit::getPos(int index) const { return fPos(index); }
 ...
 */
bool JPetHit::isSignalASet() const { return fIsSignalAset; }

/**
 * Check if signal on side B is set
 */
bool JPetHit::isSignalBSet() const { return fIsSignalBset; }

/**
 * Set the reconstruction flag with enum
 */
void JPetHit::setRecoFlag(JPetHit::RecoFlag flag) { fFlag = flag; }
...
bool JPetHit::checkConsistency() const
{
  ...
  if (slot_a != slot_b)
  {
    ERROR(Form("Signals added to Hit come from different barrel slots: %d and %d.", slot_a, slot_b));
    return false;
  }
