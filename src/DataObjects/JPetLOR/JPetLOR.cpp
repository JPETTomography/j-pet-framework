 ...
 * Default constructor
 */
JPetLOR::JPetLOR() : TObject(), fTime(0.0f), fQualityOfTime(0.0f), fTimeDiff(0.0f), fQualityOfTimeDiff(0.0f), fFlag(JPetLOR::Unknown)
{
  fIsHitSet[0] = false;
  fIsHitSet[1] = false;
}
...
JPetLOR::JPetLOR(float Time, float QualityOfTime, JPetHit& firstHit, JPetHit& secondHit)
    : TObject(), fTime(Time), fQualityOfTime(QualityOfTime), fTimeDiff(0.0f), fQualityOfTimeDiff(0.0f), fFirstHit(firstHit), fSecondHit(secondHit)
{
  fIsHitSet[0] = true;
  fIsHitSet[1] = true;
}
 ...
 */
const JPetHit& JPetLOR::getFirstHit() const { return fFirstHit; }

/**
 * Get the hit, that is second in time
 */
const JPetHit& JPetLOR::getSecondHit() const { return fSecondHit; }
 ...
 */
void JPetLOR::setQualityOfTimeDiff(const float qtd) { fQualityOfTime = qtd; }
