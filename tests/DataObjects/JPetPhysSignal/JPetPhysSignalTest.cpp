...
BOOST_AUTO_TEST_CASE(AllSignalsTest2)
{
  ...
  BOOST_CHECK_CLOSE(phys.getRecoSignal().getShape().front().time, 501.f, epsilon);
  BOOST_CHECK_CLOSE(phys.getRecoSignal().getShape().back().amplitude, 0.43f, epsilon);
  const JPetRawSignal& raw = phys.getRecoSignal().getRawSignal();
  BOOST_CHECK_EQUAL(raw.getNumberOfPoints(JPetSigCh::Leading), 2);
  BOOST_CHECK_CLOSE(raw.getPoints(JPetSigCh::Leading, JPetRawSignal::ByThrValue).front().getThreshold(), 100.f, epsilon);
}
