...
{
  JPetScin scin(123, 50.0, 30.5, 12.3, 0.5, -12.3, 44.4);
  BOOST_REQUIRE_EQUAL(scin.getID(), 123);
  BOOST_REQUIRE_CLOSE(scin.getLength(), 50.0, epsilon);
  BOOST_REQUIRE_CLOSE(scin.getHeight(), 30.5, epsilon);
  BOOST_REQUIRE_CLOSE(scin.getWidth(), 12.3, epsilon);
  BOOST_REQUIRE_CLOSE(scin.getCenterX(), 0.5, epsilon);
  BOOST_REQUIRE_CLOSE(scin.getCenterY(), -12.3, epsilon);
  BOOST_REQUIRE_CLOSE(scin.getCenterZ(), 44.4, epsilon);
}
    ...
    switch (runID) {
      ...
      case 2:
        result = {
          ...
          {5, {
            {"id", "5"},
            {"length", "150.0"}, {"height", "115.0"}, {"width", "17.0"},
            {"xcenter", "15.0"}, {"ycenter", "-15.0"}, {"zcenter", "115.0"},
            {"slot_id", "1"}
          } }
        };
        break;
      ...
      case 4:
        result = {
          {1, { {"id", "1"},
            {"length", "50.0"}, {"height", "15.0"}, {"width", "big"},
            {"xcenter", "5.0"}, {"ycenter", "-5.0"}, {"zcenter", "15.0"},
            {"slot_id", "1"}
          } }
        };
