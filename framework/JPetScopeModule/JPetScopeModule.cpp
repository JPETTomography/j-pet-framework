...
JPetScopeModule::JPetScopeModule(const char* name, const char* title):
...
void JPetScopeModule::createOutputObjects(const char* outputFilename)
{
  ...
  if (fFiles.empty()) {
    ...
    terminate();
    fIt = fFiles.begin();
    assert(fIt != fFiles.end());
    fCurrentPosition = (*fIt).first;
    createNewWriter(fCurrentPosition);
  }
}

///wk: change the name of the method
void JPetScopeModule::createNewWriter(int position)
{
  terminate();
  INFO (Form("Creating root file for position %d", position));
...
///wk: change the name of the method
void JPetScopeModule::saveNewTreeFile()
{
  assert(fWriter);
  assert(fHeader);
  assert(fParamBank);
