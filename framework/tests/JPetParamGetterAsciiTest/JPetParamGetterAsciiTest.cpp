...
BOOST_AUTO_TEST_CASE( minimal_example_write )
{
  JPetParamGetterAscii getter("DB1.json");
		...
		std::string writtenFileName("writtenDB1.json");
  ...
  JPetParamGetterAscii regetter(writtenFileName.c_str());
		...
		JPetScin & scintillator = paramBank.getScintillator(0);
		JPetScin & rescintillator = reparamBank.getScintillator(0);
		...
		JPetBarrelSlot & barrelSlot = paramBank.getBarrelSlot(0);
		JPetBarrelSlot & rebarrelSlot = reparamBank.getBarrelSlot(0);
		...
		JPetPMCalib & pmCalib = paramBank.getPMCalib(0);
		JPetPMCalib & repmCalib = reparamBank.getPMCalib(0);
		...
		JPetLayer & layer = paramBank.getLayer(0);
		JPetLayer & relayer = reparamBank.getLayer(0);
		...
		JPetFrame & frame = paramBank.getFrame(0);
		JPetFrame & reframe = reparamBank.getFrame(0);
		...
		JPetTOMBChannel & tomb = paramBank.getTOMBChannel(0);
		JPetTOMBChannel & retomb = reparamBank.getTOMBChannel(0);
		BOOST_REQUIRE(tomb.getThreshold() == retomb.getThreshold());

		JPetTRB & trb = paramBank.getTRB(0);
		JPetTRB & retrb = reparamBank.getTRB(0);
