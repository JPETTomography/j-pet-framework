...
BOOST_AUTO_TEST_CASE(nonDefaultConstr)
{
  ...
  JPetParamManager* mgr = new JPetParamManager;
  JPetParams params(opts, mgr);
  BOOST_REQUIRE_EQUAL(params.getParamManager().get(), mgr);
...
BOOST_AUTO_TEST_CASE(copyCtr)
{
  ...
  JPetParams params(opts, mgr);
  auto params2 = params;
  BOOST_REQUIRE_EQUAL(params2.getParamManager().get(), mgr);
...
BOOST_AUTO_TEST_CASE(memoryLeaks)
{
  jpet_options_tools::OptionsStrAny opts;
  opts["blaOption"] ="value";
  JPetParams* params= new JPetParams(opts, new JPetParamManager);
